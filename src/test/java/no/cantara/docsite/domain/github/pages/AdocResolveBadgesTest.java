...
public class AdocResolveBadgesTest {

    private static final Logger LOG = LoggerFactory.getLogger(AdocResolveBadgesTest.class);

    @Test
    public void thatBadgesAreResolved() throws IOException {
        String asciiDocMarkup;
        Path readmeResource = Paths.get(CommonUtil.currentPath()).resolve("README.adoc");
        try (InputStream is = new BufferedInputStream(new FileInputStream(readmeResource.toFile()))) {
            assertNotNull(is);
            asciiDocMarkup = CommonUtil.writeInputToOutputStream(is).toString();
            LOG.trace("{}", asciiDocMarkup);
        }

        List<Extension> extensions = Arrays.asList(TablesExtension.create());
        Parser parser = Parser.builder().extensions(extensions).build();
        Node document = parser.parse(ReadmeBadgeMDFixture.BADGE_MARKDOWN);
        ReadmeMarkdownWikiTest.PrintVisitor visitor = new ReadmeMarkdownWikiTest.PrintVisitor();
        // TODO look for listItem -> para -> image: Image{destination ~ jenkins).. We need a url list match badge images..
        document.accept(visitor);

    }

    static class PrintVisitor extends AbstractVisitor {
        private static final Logger LOG = LoggerFactory.getLogger(ReadmeMarkdownWikiTest.PrintVisitor.class);
